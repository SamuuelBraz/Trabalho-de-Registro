...
// Função de login e redirecionamento para home (repetida corrigida)
document.getElementById("loginCadastro").addEventListener("submit", function(event) {
    event.preventDefault();
    var usuario = document.getElementById("usuario").value; 
    var senha = document.getElementById("senha").value;

    var storedUsers = JSON.parse(localStorage.getItem('listaUser') || '[]');
    
    let userFound = storedUsers.find(user => user.usuariocad === usuario && user.senhacad === senha);
...
document.getElementById("cadastro").addEventListener("submit", function (event) {
    ...
    if (validNome && validUsuario && validSenha && validConfirmaSenha) {
        // Se todos os campos forem válidos, salvar o usuário no localStorage
